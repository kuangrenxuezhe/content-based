...
DEFINE_string(conf, "conf/content_based.conf", "Content based model config file");
...
int main(int argc, char* argv[])
{
  ...
  status = ContentBased::openModel(opts, &content_based); 
  if (!status.ok()) {
    LOG(FATAL) << status.toString();
    return -1;
  }
  arg_t arg = {&opts, content_based};
  pthread_create(&grpc_, NULL, service_grpc, &arg);

  ServiceGlue serv_glue(content_based);
...
void* service_grpc(void* args)
{
  ...
  ServiceGrpc grpc(arg->content_based);
